  ...
  <div class="tw-h-full tw-w-full">
      ...
      <v-row class="tw-h-full tw-w-full" no-gutters>
        <v-col class="tw-relative tw-h-full">
          ...
          <OverlaySideNum
            ...
            :item-count="Math.min(list.length, 10)"
            :item-height="itemHeight"
          />
        </v-col>
    ...
    <v-row no-gutters style="height: 45px" align="center">
      ...
      <v-col>
        ...
        >
          <template #label>Preview</template>
        </v-switch>
      </v-col>
      <v-col>
        <span v-show="list.length">Total {{ list.length }} Searched</span>
      </v-col>
    </v-row>
  </div>
...
import { computed, ref, watch, reactive, onActivated, toRaw } from 'vue'
...
function getDisplaySize() {
  return Math.round(
    (document.documentElement.clientHeight - 49 - 1 - 45) / itemHeight.value
  )
}
...
const sortProps = reactive<Array<{ label: string; value: SortFieldName }>>([
  ...
  { label: 'Created', value: 'createDate' },
  { label: 'Modified', value: 'updateDate' }
])
...
async function loadFileInfo(item: BaseFileInfo) {
  ...
  fileInfo.value.thumbnail = await getThumbnail(item.path)
...
function search(query: string) {
  ...
  loading.value = true
  find(currentQuery, toRaw(searchScopeDirectories.value)).then((res) => {
...
const menuActions = reactive({
  open() {
    ...
    openPath(list.value[selectedIndex.value].path)
  },
  openInFinder() {
    if (isUndefined(selectedIndex.value)) return
    showItemInFolder(list.value[selectedIndex.value].path)
  },
  showInfo() {
    if (isUndefined(selectedIndex.value)) return
  ...
  copyName() {
    if (isUndefined(selectedIndex.value)) return
    writeText(list.value[selectedIndex.value].name)
  },
  copyPath() {
    if (isUndefined(selectedIndex.value)) return
    writeText(list.value[selectedIndex.value].path)
  },
  moveToTrash() {
    if (isUndefined(selectedIndex.value)) return
...
function quickLook() {
  ...
  previewFile(fileInfo.value.path, fileInfo.value.name)
}
